        ...
        Group {
            if model.session {
                TabView(selection: $model.tab) {
                    ...
                    NativeMessages().tag(NativeTab.messages).tabItem { Label("Messages", systemImage: "message.fill") }
...
struct NativeHome: View {
    ...
    var body: some View {
        NavigationStack { ScrollView(showsIndicators: false) { VStack(spacing: 14) { NativeHeader(); NativeHero(); if let story = model.stories.first { NativeFeature(story: story) { selected = story } }; HStack { Text("TOP STORIES").font(.system(size: 18, weight: .bold, design: .serif)); Spacer(); Text("SEE ALL").font(.system(size: 7, weight: .bold)).foregroundStyle(Look.gold) }.foregroundStyle(Look.cream); if model.stories.isEmpty { Text("No published stories yet. If you’ve got a story, we’d love to hear it.").font(.system(size: 11, design: .serif)).foregroundStyle(Look.muted).frame(maxWidth: .infinity, alignment: .leading).padding(.vertical, 10) } else { ForEach(model.stories.prefix(5)) { story in NativeStoryRow(story: story) { selected = story } } }; NativeCategories() }.padding(.horizontal, 13).padding(.bottom, 24) }.background(Look.black.ignoresSafeArea()).navigationBarHidden(true).refreshable { await model.refreshStories() }.sheet(item: $selected) { NativeStoryDetail(story: $0) } }
    }
}

struct NativeHero: View {
    @EnvironmentObject private var model: NativeAppModel
    var body: some View { ZStack(alignment: .bottomLeading) { NativeImage(url: URL(string: "https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&w=1400&q=85")).overlay(LinearGradient(colors: [.clear, .black.opacity(0.9)], startPoint: .top, endPoint: .bottom)); VStack(alignment: .leading, spacing: 7) { Text("IRISH ROOTS · WORLDWIDE STORIES").font(.system(size: 7, weight: .bold)).tracking(1.4).foregroundStyle(Look.gold); Text("LAST CALL").font(.system(size: 43, weight: .bold, design: .serif)).tracking(-2); Text("Stories From Behind the Bar").font(.system(size: 20, weight: .bold, design: .serif).italic()).foregroundStyle(Look.gold); Text("Funny. Strange. Heartwarming. Unforgettable.").font(.system(size: 10, design: .serif)).foregroundStyle(Look.cream.opacity(0.86)); Button("WRITE A STORY →") { model.tab = .write }.buttonStyle(PrimaryButton()).padding(.top, 2) }.padding(17) }.frame(height: 285).clipShape(RoundedRectangle(cornerRadius: 13)).overlay(RoundedRectangle(cornerRadius: 13).stroke(Look.gold.opacity(0.25))) }
}

struct NativeFeature: View { let story: NativeStory; let action: () -> Void; var body: some View { Button(action: action) { HStack(spacing: 0) { NativeImage(url: story.imageURL).frame(width: 118); VStack(alignment: .leading, spacing: 6) { Text(story.isSample ? "SAMPLE · STORY OF THE NIGHT" : "STORY OF THE NIGHT · LAST CALL").font(.system(size: 6, weight: .bold)).tracking(1).foregroundStyle(Look.gold); Text(story.title).font(.system(size: 16, weight: .bold, design: .serif)).multilineTextAlignment(.leading); Text(story.body).font(.system(size: 9, design: .serif)).foregroundStyle(Look.muted).lineLimit(4); Text("🍺 \(story.reactions)   ·   💬 \(story.comments)   ·   READ FULL STORY →").font(.system(size: 6, weight: .bold)).foregroundStyle(Look.gold) }.padding(11); Spacer() }.frame(height: 158).background(Look.card).clipShape(RoundedRectangle(cornerRadius: 11)).overlay(RoundedRectangle(cornerRadius: 11).stroke(Look.line)) }.buttonStyle(.plain) } }
...
struct NativeStory: Identifiable, Hashable { let id: UUID; let title: String; let body: String; let author: String; let location: String; let category: String; let reactions: Int; let comments: Int; let imageURL: URL?; let isSample: Bool; init(id: UUID = UUID(), title: String, body: String, author: String, location: String, category: String, reactions: Int, comments: Int, imageURL: URL?, isSample: Bool = false) { self.id=id; self.title=title; self.body=body; self.author=author; self.location=location; self.category=category; self.reactions=reactions; self.comments=comments; self.imageURL=imageURL; self.isSample=isSample }; init(remote: RemoteStory, reactions: Int) { self.init(id: remote.id, title: remote.title, body: remote.body, author: remote.anonymousName ?? "LAST CALL member", location: remote.location.isEmpty ? "Worldwide" : remote.location, category: remote.category, reactions: reactions, comments: 0, imageURL: URL(string: "https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&w=1200&q=82")) }; static let samples = [NativeStory(title: "The Customer Who Refused to Take the Hint", body: "Sample content — real LAST CALL stories will appear here once published.", author: "SAMPLE", location: "Dublin, Ireland", category: "Closing Time", reactions: 142, comments: 12, imageURL: URL(string: "https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&w=1200&q=82"), isSample: true), NativeStory(title: "We Definitely Shouldn’t Have Stayed for One More", body: "Sample content — real LAST CALL stories will appear here once published.", author: "SAMPLE", location: "Galway, Ireland", category: "After Hours", reactions: 112, comments: 9, imageURL: URL(string: "https://images.unsplash.com/photo-1572116469696-31de0f17cc34?auto=format&fit=crop&w=1200&q=82"), isSample: true), NativeStory(title: "Never Trust a Man Who Says ‘Just One Quick Pint’", body: "Sample content — real LAST CALL stories will appear here once published.", author: "SAMPLE", location: "Cork, Ireland", category: "Bar Wisdom", reactions: 89, comments: 7, imageURL: URL(string: "https://images.unsplash.com/photo-1527761939622-933c0a2a6a57?auto=format&fit=crop&w=1200&q=82"), isSample: true)] }
