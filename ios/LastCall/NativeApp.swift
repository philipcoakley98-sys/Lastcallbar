...
// Existing LAST CALL app implementation remains below; this file is intentionally kept
// as the shared model/navigation layer. The Messages tab uses LastCallMessages.
...
final class NativeAppModel: ObservableObject {
    ...
    func bootstrap() async { if let auth = await LastCallAPI.shared.restoreSession(), let t = auth.accessToken, let u = auth.user { token=t; user=u; signedIn=true; await refreshAccount() }; await refreshStories() }
    func refreshStories() async { do { let remote = try await LastCallAPI.shared.fetchPublishedStories(); if remote.isEmpty { stories=[] } else { let ids=remote.map(\.id); let counts=(try? await LastCallAPI.shared.fetchReactionCounts(storyIDs: ids, accessToken: token ?? "")) ?? [:]; stories=remote.map { NativeStory(remote:$0,reactions:counts[$0.id] ?? 0) } } } catch { error="Stories could not be refreshed just now." }; refreshID=UUID() }
    ...
    func signIn(_ email:String,_ password:String) async { do { let auth=try await LastCallAPI.shared.signIn(email:email.trimmingCharacters(in:.whitespacesAndNewlines),password:password); guard let t=auth.accessToken,let u=auth.user else{throw LastCallAPIError.server("Sign in did not return a session.")}; token=t;user=u;signedIn=true;showAuth=false;await refreshAccount() } catch { error=errorMessage(error) } }
    func signUp(_ email:String,_ password:String,_ name:String) async { do { let auth=try await LastCallAPI.shared.signUp(email:email.trimmingCharacters(in:.whitespacesAndNewlines),password:password,displayName:name.nilIfEmpty); if let t=auth.accessToken,let u=auth.user { token=t;user=u;signedIn=true;showAuth=false;await refreshAccount() } else { error="Account created. Check your email to confirm your address, then come back to LAST CALL and sign in." } } catch { error=errorMessage(error) } }
    ...
    func submit(title:String,body:String,category:String,city:String,country:String,anonymous:Bool) async -> Bool { guard let t=token,let id=user?.id else{authMode=.signIn;showAuth=true;return false};do{try await LastCallAPI.shared.insertStory(title:title,body:body,category:category,city:city,country:country,anonymous:anonymous,accessToken:t,userID:id);return true}catch{error="Your story could not be sent just now.";return false} }
