...
final class LastCallAPI {
    ...
    func fetchReactionStoryIDs(accessToken: String) async throws -> Set<UUID> {
        var components = URLComponents(url: baseURL.appendingPathComponent("rest/v1/reactions"), resolvingAgainstBaseURL: false)!
        components.queryItems = [URLQueryItem(name: "select", value: "story_id"), URLQueryItem(name: "user_id", value: "eq.current_user"), URLQueryItem(name: "kind", value: "eq.beer")]
        var request = URLRequest(url: components.url!)
        request.setValue(publishableKey, forHTTPHeaderField: "apikey")
    ...
    func toggleBeerReaction(storyID: UUID, accessToken: String, reacted: Bool) async throws -> Bool {
        var components = URLComponents(url: baseURL.appendingPathComponent("rest/v1/reactions"), resolvingAgainstBaseURL: false)!
        components.queryItems = [URLQueryItem(name: "story_id", value: "eq.\(storyID.uuidString)"), URLQueryItem(name: "kind", value: "eq.beer")]
        ...
        if reacted {
            ...
            request = self.request(path: "rest/v1/reactions", method: "POST", body: try JSONEncoder.lastCall.encode(ReactionInsert(storyId: storyID, kind: "beer")), accessToken: accessToken)
    ...
    func signUp(email: String, password: String, displayName: String?) async throws -> AuthResponse {
        ...
        let request = self.request(path: "auth/v1/signup", method: "POST", body: data)
        let (responseData, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: responseData)
        let auth = try JSONDecoder.lastCall.decode(AuthResponse.self, from: responseData)
        persist(auth)
        return auth
    }
    ...
    func signIn(email: String, password: String) async throws -> AuthResponse {
        ...
        let data = try JSONEncoder.lastCall.encode(payload)
        let request = self.request(path: "auth/v1/token?grant_type=password", method: "POST", body: data)
        let (responseData, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: responseData)
        let auth = try JSONDecoder.lastCall.decode(AuthResponse.self, from: responseData)
        persist(auth)
    ...
    func restoreSession() async -> AuthResponse? {
        ...
        do {
            let body = try JSONEncoder.lastCall.encode(RefreshPayload(refreshToken: refreshToken))
            let request = self.request(path: "auth/v1/token?grant_type=refresh_token", method: "POST", body: body)
            let (data, response) = try await URLSession.shared.data(for: request)
            try validate(response, data: data)
    ...
    func toggleFollow(targetID: UUID, userID: UUID, accessToken: String, following: Bool) async throws -> Bool {
        ...
        if following {
            ...
            request = self.request(path: "rest/v1/follows", method: "POST", body: try JSONEncoder.lastCall.encode(FollowInsert(followerId: userID, followingId: targetID)), accessToken: accessToken)
            request.setValue("return=minimal", forHTTPHeaderField: "Prefer")
        }
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
    ...
    func startConversation(targetID: UUID, accessToken: String) async throws -> UUID {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
        return try JSONDecoder.lastCall.decode(UUID.self, from: data)
    }
    ...
    func fetchConversations(userID: UUID, accessToken: String) async throws -> [ConversationRow] {
        ...
        for membership in memberships {
            let members = try await fetchOtherMember(conversationID: membership.conversationId, userID: userID, accessToken: accessToken)
            guard let other = members.first, let otherUserID = other.userId, let profile = try await fetchProfile(userID: otherUserID, accessToken: accessToken) else { continue }
            let latest = try await fetchLatestMessage(conversationID: membership.conversationId, accessToken: accessToken)
            result.append(ConversationRow(id: membership.conversationId, status: membership.status, otherUser: profile, latestMessage: latest))
        }
        return result
    }
    ...
    func fetchMessages(conversationID: UUID, accessToken: String) async throws -> [MessageRow] {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
        return try JSONDecoder.lastCall.decode([MessageRow].self, from: data)
    }
    ...
    func fetchNotifications(userID: UUID, accessToken: String) async throws -> [NotificationRow] {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
        return try JSONDecoder.lastCall.decode([NotificationRow].self, from: data)
    }
    ...
    func markNotificationsRead(userID: UUID, accessToken: String) async throws {
        ...
        request.setValue("return=minimal", forHTTPHeaderField: "Prefer")
        request.httpBody = try JSONEncoder.lastCall.encode(["read_at": ISO8601DateFormatter().string(from: Date())])
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
    }
    ...
    private func fetchOtherMember(conversationID: UUID, userID: UUID, accessToken: String) async throws -> [MembershipRow] {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
        return try JSONDecoder.lastCall.decode([MembershipRow].self, from: data)
    }
    ...
    private func fetchLatestMessage(conversationID: UUID, accessToken: String) async throws -> MessageRow? {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response, data: data)
        return try JSONDecoder.lastCall.decode([MessageRow].self, from: data).first
    }

    private func persist(_ auth: AuthResponse) { if let refreshToken = auth.refreshToken { keychainSet(refreshToken) } }
    func accessTokenFrom(_ auth: AuthResponse) -> String? { auth.accessToken }
...
struct ReactionInsert: Encodable { let storyId: UUID; let kind: String }
