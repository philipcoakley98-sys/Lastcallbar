...
struct NativeStoryDetail: View {
  ...
  @State private var authors: [UUID: ProfileRow] = [:]
        ...
        Section("PROFILE PHOTO") {
          HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
              ...
              Text("Tap the photo to choose a new one.")
                .font(.system(size: 9))
                .foregroundStyle(Look.muted)
            }
          }
...
struct NativeNotifications: View {
  ...
  private func load() async {
    guard let token = model.token, let userID = model.user?.id else { loading = false; return }
    do {
  ...
  private func title(for type: String) -> String {
    ...
    if value.contains("reaction") || value.contains("beer") { return "Someone raised a pint to your story." }
