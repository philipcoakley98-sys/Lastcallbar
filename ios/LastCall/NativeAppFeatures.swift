...
extension LastCallAPI {
    ...
    func fetchReactionCounts(storyIDs: [UUID], accessToken: String) async throws -> [UUID: Int] {
        ...
        var components = URLComponents(url: nativeBaseURL.appendingPathComponent("rest/v1/reactions"), resolvingAgainstBaseURL: false)!
    ...
    func markMessageRead(messageID: UUID, accessToken: String) async throws {
        ...
        var request = URLRequest(url: nativeBaseURL.appendingPathComponent("rest/v1/rpc/mark_message_read"))
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw LastCallAPIError.server(String(data: data, encoding: .utf8) ?? "Message could not be marked read.")
        }
    }
    ...
    func blockUser(userID: UUID, accessToken: String) async throws {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw LastCallAPIError.server(String(data: data, encoding: .utf8) ?? "That member could not be blocked.")
        }
    }
}
